import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { ILineaPedido, LineaPedido } from 'app/shared/model/linea-pedido.model';
import { LineaPedidoService } from './linea-pedido.service';
import { LineaPedidoComponent } from './linea-pedido.component';
import { LineaPedidoDetailComponent } from './linea-pedido-detail.component';
import { LineaPedidoUpdateComponent } from './linea-pedido-update.component';

@Injectable({ providedIn: 'root' })
export class LineaPedidoResolve implements Resolve<ILineaPedido> {
  constructor(private service: LineaPedidoService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ILineaPedido> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((lineaPedido: HttpResponse<LineaPedido>) => {
          if (lineaPedido.body) {
            return of(lineaPedido.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new LineaPedido());
  }
}

export const lineaPedidoRoute: Routes = [
  {
    path: '',
    component: LineaPedidoComponent,
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'LineaPedidos'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/view',
    component: LineaPedidoDetailComponent,
    resolve: {
      lineaPedido: LineaPedidoResolve
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'LineaPedidos'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: 'new',
    component: LineaPedidoUpdateComponent,
    resolve: {
      lineaPedido: LineaPedidoResolve
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'LineaPedidos'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/edit',
    component: LineaPedidoUpdateComponent,
    resolve: {
      lineaPedido: LineaPedidoResolve
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'LineaPedidos'
    },
    canActivate: [UserRouteAccessService]
  }
];
