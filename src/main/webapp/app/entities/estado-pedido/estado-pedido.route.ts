...
export class EstadoPedidoResolve implements Resolve<IEstadoPedido> {
  constructor(private service: EstadoPedidoService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IEstadoPedido> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((estadoPedido: HttpResponse<EstadoPedido>) => {
